...
router.get('/data/departments',authenticateToken,async(req,res)=>{
    try{
        const {data,error}=await supabase.from('departments').select(`
            ...
            description,
            is_active
        `).eq('is_active',true).order('department_name');
...
router.get('/data/subjects',authenticateToken,async(req,res)=>{
    try{
        const {data,error}=await supabase.from('subjects').select(`
            ...
            department_id,
            is_active
        `).eq('is_active',true).order('school_level').order('subject_name');
...
router.get('/data/secondary-classes',authenticateToken,async(req,res)=>{
    try{
        const {data,error}=await supabase.from('classes').select(`
            ...
            academic_year_id,
            is_active
        `).eq('school_section','Secondary').eq('is_active',true)
          .order('class_name').order('arm');
...
router.post('/department-heads',authenticateToken,requireRoles('Administrator','Principal','Vice Principal'),async(req,res)=>{
    try{
        ...
        const {data,error}=await supabase.from('department_heads').insert([{
            department_id,
            teacher_id,
            academic_year_id
        }]).select().single();
...
router.get('/secondary/class-masters',authenticateToken,async(req,res)=>{
    try{
        ...
        const {data,error}=await supabase.from('secondary_class_masters').select(`
            assignment_id,
            class_id,
            teacher_id,
            academic_year_id,
...
router.post('/secondary/class-masters',authenticateToken,requireRoles('Administrator','Principal','Vice Principal'),async(req,res)=>{
    try{
        ...
        const {data,error}=await supabase.from('secondary_class_masters').insert([{
            class_id,
            teacher_id,
            academic_year_id
        }]).select().single();
...
router.get('/secondary-assignments',authenticateToken,async(req,res)=>{
    try{
        ...
        const secondaryAssignments=(data||[]).filter(
            item=>item.classes?.school_section==='Secondary'
        );
        ...
        res.status(500).json({
            message:'Failed to load secondary teacher assignments'
        });
    }
});
...
router.post('/secondary-assignments',authenticateToken,requireRoles('Administrator','Principal','Vice Principal'),async(req,res)=>{
    try{
...
router.post('/',authenticateToken,requireRoles('Administrator','Principal','Vice Principal'),async(req,res)=>{
    try{
        const {
            staff_number,
            first_name,
...
router.put('/:teacherId',authenticateToken,requireRoles('Administrator','Principal','Vice Principal'),async(req,res)=>{
    try{
        const teacherId=Number(req.params.teacherId);
...
router.get('/:teacherId',authenticateToken,async(req,res)=>{
    try{
        ...
        const {data,error}=await supabase.from('teachers').select(`
            ...
            school_section,
            employment_type
        `).eq('teacher_id',teacherId).single();
